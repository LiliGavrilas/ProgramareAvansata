...
public class Main {
    public static void main(String[] args) {
        ...
        DataBase db =DataBase.get_DB("jdbc:mysql://192.168.1.9:3306/MusicAlbums","dba","sql");
        ArtistController artistController = new ArtistController(db);
        AlbumController albumController =new AlbumController(db);
        Artist artist =new Artist("testE", "Romania");//eu
        Album album = new Album("albumtest",8, 2020);
        ...
        try {
            System.out.println(albumController.findByArtist(5));
        }
        catch (Exception e)
        {
            e.printStackTrace();
